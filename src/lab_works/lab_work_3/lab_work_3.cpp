...
namespace M3D_ISICG
{
	...
	bool LabWork3::init()
	{
		...
		_cube.iSommets = { 0, 1, 3, 1, 2, 3, 1, 5, 6, 1, 2, 6, 4, 5, 6, 4, 7, 6,
	...
	void LabWork3::_initBuffers()
	{
		...
		glNamedBufferData(
			_cube._vboPositions, _cube.pSommets.size() * sizeof( glm::vec3 ), _cube.pSommets.data(), GL_STATIC_DRAW );
		glCreateBuffers( 1, &_cube._vboColors );
		glNamedBufferData(
			_cube._vboColors, _cube.pSommets.size() * sizeof( glm::vec3 ), _cube.pSommets.data(), GL_STATIC_DRAW );
		...
		glNamedBufferData( _cube.ebo,
						   _cube.iSommets.size() * sizeof( unsigned int ),
						   _cube.iSommets.data(),
						   GL_STATIC_DRAW );
	...
	LabWork3::Mesh LabWork3::_createCube()
	{
		...
		cube._transformation = transformation;
